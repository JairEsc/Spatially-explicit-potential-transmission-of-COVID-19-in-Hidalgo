# -*- coding: utf-8 -*-
"""
Modified on Wed Sep 30 12:22:10 2020
...
def generate_demographic_contact_network(N, demographic_data, layer_generator='LFR', layer_info=None, distancing_scales=[], isolation_groups=[]):
    ...
    print(list(household_size_distn_givenGT1.values()))
    ...
    ageBrackets_NOTU20           = ['20-29', '30-39', '40-49', '50-59', '60-69', '70-79']
    ...
    householdSituations_prob['NOTu20_o60_eq1']    = pctHouseholdsWithMember_O60_givenEq1
    householdSituations_prob['NOTu20_NOTo60_eq1'] = (1 - pctHouseholdsWithMember_O60_givenEq1-householdSituations_prob['u20_NOTo60_eq1'])*prob_eq1
    householdSituations_prob['u20_o60_gt1']       = pctHouseholdsWithMember_U20andO60
    householdSituations_prob['u20_NOTo60_gt1']    = prob_u20 - householdSituations_prob['u20_o60_gt1'] - householdSituations_prob['u20_NOTo60_eq1']
    householdSituations_prob['NOTu20_o60_gt1']    = prob_o60 - householdSituations_prob['u20_o60_gt1'] - householdSituations_prob['NOTu20_o60_eq1'] 
    householdSituations_prob['NOTu20_NOTo60_gt1'] = 1-householdSituations_prob['u20_NOTo60_eq1'] -householdSituations_prob['NOTu20_o60_eq1']-householdSituations_prob['NOTu20_NOTo60_eq1']-householdSituations_prob['NOTu20_o60_gt1']  -householdSituations_prob['u20_NOTo60_gt1']-householdSituations_prob['u20_o60_gt1'] 
    print(numpy.sum(list(householdSituations_prob.values())))
    ...
    if(layer_info is None):
        ...
        layer_info  = { '0-9':   {'ageBrackets': ['0-9'],                   'meanDegree': 8.6, 'meanDegree_CI': (0.0, 17.7) },
                        ...
                        '60+':   {'ageBrackets': ['60-69', '70-79'], 'meanDegree': 13.9, 'meanDegree_CI': (7.3, 20.5) } }
        ...
        while(not graph_generated):
            try:
                if(layer_generator == 'LFR'):
                    ...
                    layerInfo['graph'] = networkx.LFR_benchmark_graph(
...
def household_country_data(country):
    ...
    if(country=='US'):
        household_data = {
                           ...
                           'age_distn':{'0-9':   (247472+275907)/2858359,
                                                '10-19': (280973+252974)/2858359,
                                                '20-29': (244185+206604)/2858359,
                                                '30-39': (211989+201292)/2858359,
                                                '40-49': (294610+161899)/2858359,
                                                '50-59': (148389+116149)/2858359,
                                                '60-69': (93750+74311)/2858359,
                                                '70-79': (56610+86827)/2858359 },

                            'household_stats':{ 'pct_with_under20':          0.55, 
                                                'pct_with_over60':           0.77,
                                                'pct_with_under20_over60':  0.44,
                                                'pct_with_over60_givenSingleOccupant':       0.0775,
                                                'mean_num_under20_givenAtLeastOneUnder20':  2.04 }
                        }

    return household_data
numNodes = 10850
baseGraph    = generate_demographic_contact_network(numNodes, household_country_data('US'), layer_generator='LFR')


model = SEIRSNetworkModel(G=baseGraph[0]['baseline'], beta=0.626, sigma=.19, gamma=.344,
                         
                          initI=2)

#checkpoints = {'t': [20, 200], 'G': [G_distancing, baseGraph[0]['baseline']], 'p': [0.1, 0.5], 'theta_E': [0.02, 0.02], 'theta_I': [0.02, 0.02], 'phi_E':   [0.2, 0.2], 'phi_I':   [0.2, 0.2]}
